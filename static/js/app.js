...
async function initializeApp() {
    ...
    // YouTube 검색 결과 예제 데이터 로드
    loadSampleYouTubeResults();
...
// YouTube 검색 결과 예제 데이터 로드
function loadSampleYouTubeResults() {
    // 실제로는 YouTube Data API를 사용하지만, 이 데모에서는 예제 데이터 사용
    const sampleResults = [
        {
            id: "dQw4w9WgXcQ",
        ...
        {
            id: "_M2wHBzLn4E",
            title: "Taylor Swift - Style (Official Video)",
            thumbnail: "https://i.ytimg.com/vi/_M2wHBzLn4E/mqdefault.jpg",
...
async function searchYouTube() {
    ...
    try {
        ...
        if (!apiKey || apiKey === 'YOUR_YOUTUBE_API_KEY') {
            console.warn("YouTube API 키가 설정되지 않았습니다. 샘플 데이터를 사용합니다.");
            loadSampleYouTubeResults();
            return;
        }
        ...
        if (data.error) {
            ...
            searchResultsContainer.innerHTML = `<p>검색 오류: ${data.error.message}</p>`;
            return;
        }
...
function displaySearchResults(results) {
    ...
    results.forEach(video => {
        ...
        // 비디오 항목 클릭 이벤트
        item.querySelector('.video-item').addEventListener('click', function() {
